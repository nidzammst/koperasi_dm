...
const santriQuery = async (queryParams) => {
  try {
    let query = {};
    ...
    const mongooseQuery = Santri.find(query)

    // Menangani sorting
    /* Pengunaan */
    /* {{base_url}product/products?sort=sold */
    if (queryParams.sort) {
...
const vendorQuery = async (queryParams) => {
  try {
    let query = {};
    ...
    // Membuat query Mongoose
    const mongooseQuery = Vendor.find(query)
...
const getProducts = async (queryParams) => {
  try {
    ...
    if (queryParams.search) {
      ...
      const accountQuery = await Vendor.find({ fullname: searchRegExp });
...
const getPaymentHistory = async (queryParams) => {
  try {
    let query = {};
    ...
    if (queryParams.search) {
      const searchRegExp = new RegExp(queryParams.search, 'i'); // buat search params case insensitive
      // Mencari di tabel Account berdasarkan fullname
      const accountQuery = await Santri.find({ fullname: searchRegExp });
      const accountIds = accountQuery.map(account => account._id);
      query.$or = [ // menggunakan operator or agar search bisa lebih dari satu dalam satu query
    ...
    // Membuat query Mongoose
    const mongooseQuery = Payment.find(query)
      .populate('account')
